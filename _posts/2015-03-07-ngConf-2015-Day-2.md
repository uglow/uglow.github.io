...
## Badges? We don't need no stinkin' badges! - Lukas Ruebbelke & Geoff Goodman (, wrote Plunkr) - 2:35pm
...
- Reactive: `var cStream = Observable.combineLatest(aStream, bStream, function (a, b) { return a + b; });` 

### We can use this today
- Library: rxjs.js ???
- Module: `['rx']`
- `var disposable = $scope.$createObservableFunction('increaseCounter').flatMap(fn).do(apiService.method).scan(fn).subscribe(fn);`
